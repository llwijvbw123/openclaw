...
loading. For a persistent elevation host, use the stronger canonical mode and its deployment-owned launchd job:
...
Settings to grant it. The installer uses the separate `ai.openclaw.mac.elevation-host` job and refuses to race or
rewrite ordinary **Launch at login** (`ai.openclaw.mac`).
