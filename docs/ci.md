...
| `blacksmith-8vcpu-ubuntu-2404`  | Retained heavy Linux Node suites, compact-small queue-tail bins 2, 5, and 8, first-attempt same-repo `checks-ui-e2e` rows, the three hybrid core-lint stripes, boundary/extension-heavy `check-additional-*` shards, `check-sqlite-session-lifecycle`, and `android`                                                                                                                                                                                                                                                                                     |
...
Heavy lanes are `build-artifacts`, `check-sqlite-session-lifecycle`, `android`, `macos-node`, `macos-swift`, and `ios-build`. Hybrid also sends the eight-row hosted-planner `checks-ui-e2e` matrix, six-row QA Smoke matrix, `check-lint`, three core-lint stripes, `check-test-types`, and `check-additional-extension-package-boundary` to their historical Blacksmith sizes on attempt 1. Every other configurable `ci.yml` lane stays hosted in hybrid, including preflight, the Node test matrix, the real-Gateway UI E2E job, Windows, the remaining lint/check rows, docs, and Python skills. Separate Opengrep workflows remain GitHub-hosted.
...
The changed-target PR plan reduces the common Node test burst from 29 Blacksmith registrations to one. Broad-risk PRs keep the 29-registration metadata-complete compact fallback; canonical pushes use 24 nondist compact registrations. With the `github` or `hybrid` hosted planner profile, the broader fallback uses 52 nondist hosted jobs and canonical pushes use 45; these jobs do not consume Blacksmith runner registrations. The widened hybrid attempt-1 set adds 20 Blacksmith registrations per full run: eight UI E2E rows, six QA Smoke rows, three core-lint stripes, and three singleton checks.
